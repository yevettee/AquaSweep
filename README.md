...
AquaSweep는 철갑상어 양식장을 NVIDIA Isaac Sim 위에 재현하고, ROS 2로 제어되는 자율 로봇들이
수조 바닥과 벽을 청소하며 죽은 물고기를 수거하도록 구성한 멀티 로봇 시뮬레이션입니다. 카메라
영상에서 물고기와 이물질을 인지하고, 수조 단위로 청소 작업을 계획·실행하며, 전체 상태를 GUI
대시보드로 시각화합니다.

세 종류의 로봇이 협업하며, 수조 7개 환경에 **총 15대**가 배치됩니다.

- **Hippo** (7대, 수조당 1대) — 수조 바닥을 주행하며 이물질을 흡입하는 수중 청소 로봇
