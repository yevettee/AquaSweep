...
class PoolState:
    ...
    def __init__(self, classifier_config: Optional[Dict] = None):
        ...
        self.status_classifier = ActivityFishStatusClassifier(
            history_size=cls_cfg.get('history_size', 30),
            activity_threshold=cls_cfg.get('activity_threshold', 3.0),
        )
...
class FishDetectionNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('yolo.debris_min_area', 3)
        self.declare_parameter('yolo.debris_max_area', 8)
        ...
        self.get_logger().info(
            f"Classifier: suspicious when activity (Act) <= "
            f"{self._classifier_config['activity_threshold']}"
        )
    ...
    def _create_detector(self) -> FishYOLODetector:
        ...
        detector = FishYOLODetector(
            ...
            iou_threshold=self.get_parameter('yolo.iou_threshold').value,
            imgsz=self.get_parameter('yolo.imgsz').value,
            half=self.get_parameter('yolo.half').value,
            use_tracking=False,
            debris_min_area=self.get_parameter('yolo.debris_min_area').value,
            debris_max_area=self.get_parameter('yolo.debris_max_area').value,
            debris_debug=False,
        )
        detector.warmup()
        if not detector._initialized:
            raise RuntimeError(
                f"YOLO init failed — check ultralytics install and "
                f"{self.get_parameter('yolo.model_path').value}"
            )
        return detector
