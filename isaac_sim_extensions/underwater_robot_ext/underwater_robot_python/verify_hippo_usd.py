...
"""hippo_v1.usd 구조·치수가 global_variables 와 일치하는지 오프라인 검증 (Isaac Sim 불필요)."""
...
_EXT_DATA = Path(__file__).resolve().parents[1] / "data" / gv.HIPPO_USD_FILENAME
...
def main() -> int:
    ...
    hippo = stage.GetPrimAtPath("/Root/hippo")
    if not hippo.IsValid():
        print("FAIL: /Root/hippo not found")
        return 1
    ...
    for prim in Usd.PrimRange(hippo):
    ...
    elif abs(wheel_radius - gv.HIPPO_WHEEL_RADIUS_M) > 0.002:
        errors.append(f"wheel_radius {wheel_radius} != {gv.HIPPO_WHEEL_RADIUS_M}")
    ...
    if len(wheel_pos) != 2:
        ...
        if abs(wb - gv.HIPPO_WHEEL_BASE_M) > 0.01:
            errors.append(f"wheel_base {wb:.4f} != {gv.HIPPO_WHEEL_BASE_M}")
    ...
    for joint_name in ("left_wheel_joint", "right_wheel_joint"):
        found = any(p.GetName() == joint_name for p in Usd.PrimRange(hippo))
        if not found:
    ...
    print("OK: hippo_v1.usd matches global_variables")
