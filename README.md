# AquaSweep — 양식장 자율 청소 로봇 시뮬레이션

NVIDIA Isaac Sim 위에 철갑상어 양식장(원통형 수조 7개)을 재현하고, ROS 2로 제어되는 자율 로봇들이
바닥과 벽을 청소하며 죽은 물고기를 수거하는 멀티 로봇 시뮬레이션입니다. 카메라 영상에서 물고기와
이물질을 인지하고, 풀 단위로 청소 작업을 계획·실행하며, PyQt5 대시보드로 전체 상태를 시각화합니다.
...
> 경로 예시는 `~/water_ws/src` 를 저장소 루트로 가정합니다.

### 6.1 ROS 2 워크스페이스 빌드 (Python 3.10)

```bash
cd ~/water_ws/src/water_ws
colcon build
...
1. Isaac Sim을 실행하고 `aquasweep_ext` 익스텐션을 활성화합니다.
   (Window ▸ Extensions에서 `isaac_sim_extensions/` 경로를 ext-folder로 추가)
...
source ~/water_ws/src/water_ws/install/setup.bash
